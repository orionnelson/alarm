...
void *display_thread_1(void *arg)
{
	...
	{
        fprintf(stdout, "Display Thread 1: Number of SecondsLeft <%d>: Alarm Request Number: (%d) Alarm Request: <%s> \n", alarm->time - time (NULL), alarm->Alarm_Request_Number,alarm->message);
	    sleep(2);
	}
...
int main (int argc, char *argv[])
{
    ...
    while (1) {
        ...
        /* Check Standard Input and Make Sure that all standard input args are carried across start the program after stdin is set to line and iterated on each version of i;
        */
        int sz = argc;
        if (sz > 1 && sz > (i+1)){
        ...
        */
	    ...
	    /* Alarm request received message */
            ...
            fprintf(stdout,"Main Thread Received Alarm Request Number: (%d) Alarm Request: <%s> \n", alarm->Alarm_Request_Number, alarm->message);
            /*
